...
tracing_events_num_str = {
    ...
    251: "SA_PHY_SR",
    259: "SA_RESERVED",
    260: "PHY_TX_PKT_START",
    261: "PHY_TX_PKT_END",
    262: "PHY_TX_CTX_START",
    263: "PHY_TX_CTX_END",
    264: "PHY_TX_RX_CAL_START",
    265: "PHY_TX_RX_CAL_END",
    266: "PHY_RX_PKT_START",
    267: "PHY_RX_PKT_END",
    268: "PHY_RX_CAL_START",
    269: "PHY_RX_CAL_END",
    271: "ELG",
    272: "ELG_TIMER",
    273: "ELG_EVENT",
...
def my_wait(sec, new_count=0):
    ...
    for i in range(sec):
        # if(glob['QUIT']):
        #     break
        lines = "[{} lines decoded, lastCL {}]".format(new_count, cl_id)
...
def graph_cl_timings():
    ...
    # rx cal duration after tx
    phy_timings_df = phy_timings_df.assign(dummy_frt=pd.Series(np.nan))
    phy_timings_df['dummy_frt'] = phy_timings_df.frt_dec.shift(-1)
    phy_timings_df = phy_timings_df.assign(rxCalDurInTx=pd.Series(np.nan))
    phy_timings_df['rxCalDurInTx'] = np.where(((phy_timings_df.tracecode_dec == tracing_events_str_num['PHY_TX_RX_CAL_START'])), phy_timings_df.dummy_frt-phy_timings_df.frt_dec, np.nan)
    phy_timings_df.drop(columns=['dummy_frt'], inplace=True)
    # freq
    rxCalDurInTx_df = phy_timings_df[['byte', 'rxCalDurInTx']].groupby('rxCalDurInTx').agg('count').rename(columns={'byte': 'freq'}).reset_index()
    rxCalDurInTx_df = rxCalDurInTx_df.astype({'rxCalDurInTx': int})
    # PDF
    rxCalDurInTx_df['pdf'] = rxCalDurInTx_df['freq'] / sum(rxCalDurInTx_df['freq'])
    # CDF
    rxCalDurInTx_df['cdf'] = rxCalDurInTx_df['pdf'].cumsum()
    rxCalDurInTx_df = rxCalDurInTx_df.reset_index()
    ...
    names_label_dic = {
        ...
        'TxPHR1': "TxPHR",
        'EndTxTime': "EndTxTime",
        'rxCalStartInTx': 'rxCalStartInTx',
        'rxCalEndInTx': 'rxCalEndInTx',
    ...
    colors_dic = {
        ...
        'TxPHR1': 'crimson',
        'EndTxTime': 'crimson',
        'rxCalStartInTx': 'royalblue',
        'rxCalEndInTx': 'royalblue',
    ...
    diff_points_dic = {
        ...
        'rxCalinTx': int(phy_timings_df.rxCalDurInTx.mean()),
    ...
    points_dic['rxCalEndInTx'] = points_dic['EndTxTime'] + diff_points_dic['rxCalinTx']
    ...
    arrows_x = [
        ...
        {'x1': points_dic['EndTxTime'], 'x2': points_dic['rxCalEndInTx'], 'y': 1.6},
    ...
    val_texts = [
        ...
        '1000(to have)',
        str(int(phy_timings_df.rxCalDurInTx.min()))+'/'+str(int(phy_timings_df.rxCalDurInTx.max()))+'/'+str(int(phy_timings_df.rxCalDurInTx.mean())),
    ...
    if GRAPH_OPTION == GRAPH_PLOTLY:
        ...
        write_df_to_json("cltimings_rxCalDurInTxJson", rxCalDurInTx_df)
...
def graph_timeline_visualiser():
    ...
    phy_trace_list = [*range(260, 270)]
    filter_list.extend(phy_trace_list)
...
def graph_it():
    ...
    cl_filter_list.extend(list(range(260, 270)))
    ...
    if '0' in graph_ans_list or '2' in graph_ans_list or '3' in graph_ans_list:
        while True:
            ...
            phy_filter_list = [*range(260, 270)]
            phy_timings_df = cl_csv_df[cl_csv_df.tracecode_dec.isin(phy_filter_list)]
            # filter_list.extend(phy_trace_list)
            timings_df = cl_csv_df[cl_csv_df.tracecode_dec.isin(filter_list)]
            # nextCLI
